...
def sliding_average_frequency(x: torch.Tensor, window: int) -> torch.Tensor:
    ...
    y = F.avg_pool1d(y, kernel_size=window, stride=1, padding=window // 2, count_include_pad=True)
...
def make_progressive_targets(
    ...
    for window in windows:
        if window <= 1:
            ...
            mag = (mag_x + sliding_average_frequency(residual, int(window))).clamp_min(0.0)
        spec = mag * phase_y
...
class MultiResolutionSTFTLoss(nn.Module):
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        for n_fft in self.fft_sizes:
            ...
            target_spec = stft_complex(target, n_fft=n_fft, hop_length=hop, win_length=win)
            pred_mag = pred_spec.abs()
            target_mag = target_spec.abs()
            sc = torch.linalg.vector_norm(pred_mag - target_mag) / torch.linalg.vector_norm(target_mag).clamp_min(self.eps)
            log_mag = F.l1_loss(torch.log(pred_mag + self.eps), torch.log(target_mag + self.eps))
...
class LogSpectralDistance(nn.Module):
    ...
    def __init__(self, n_fft: int = 1536, hop_length: int = 768, win_length: int = 1536, eps: float = 1.0e-7) -> None:
        super().__init__()
        self.n_fft = n_fft
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        return torch.sqrt(torch.mean((pred_db - target_db) ** 2, dim=1).clamp_min(self.eps)).mean()
...
class StageReconstructionLoss(nn.Module):
    ...
    def forward(
        ...
        if len(pred_stages_ri) != len(self.windows):
            raise ValueError(f"Got {len(pred_stages_ri)} predictions but {len(self.windows)} windows")
        ...
        for idx, (pred_ri, target_stage_ri) in enumerate(zip(pred_stages_ri, target_stages_ri)):
            ...
            if self.weights.complex_l1:
                loss = loss + self.weights.complex_l1 * complex_l1(pred_ri, target_stage_ri)
            total = total + loss
